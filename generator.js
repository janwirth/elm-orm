...
var $author$project$Generator$extractTypeAliases = function (file) {
	return A2(
		...
		function (_v0) {
			...
			if (declaration.$ === 'AliasDeclaration') {
				...
				return $elm$core$Maybe$Just(typeAlias);
			} else {
				return $elm$core$Maybe$Nothing;
			}
		},
...
var $author$project$Generator$extractFields = function (typeAlias) {
	var _v0 = typeAlias.typeAnnotation;
	if (_v0.b.$ === 'Record') {
		var recordDefinition = _v0.b.a;
		return A2(
			$elm$core$List$map,
			function (_v1) {
				var _v2 = _v1.b;
...
var $author$project$Generator$generateCreateTable = function (typeAlias) {
	var typeName = function (_v0) {
		var name = _v0.b;
		return name;
	}(typeAlias.name);
	var tableName = $elm$core$String$toLower(typeName) + 's';
	var fields = $author$project$Generator$extractFields(typeAlias);
	var fieldColumns = A2(
		$elm$core$String$join,
		',\n',
		A2(
	...
	var allColumns = A2(
		...
		A2(
			...
			_List_fromArray(
				['        id INTEGER PRIMARY KEY AUTOINCREMENT', fieldColumns, '        created_at DATETIME DEFAULT CURRENT_TIMESTAMP', '        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP'])));
	var createTableSql = A2(
		$elm$core$String$join,
		'\n',
		_List_fromArray(
			[tableName + 'CreateTable : String', tableName + 'CreateTable =', '    \"\"\"CREATE TABLE IF NOT EXISTS ' + (tableName + ' ('), allColumns, '    )\"\"\"']));
	return createTableSql;
};
var $author$project$Generator$generateMigrations = function (typeAliases) {
	var sqlStatements = A2(
		$elm$core$String$join,
		'\n\n',
		A2($elm$core$List$map, $author$project$Generator$generateCreateTable, typeAliases));
	var migrationsHeader = 'module Migrations exposing (..)\n\n';
	return _Utils_ap(migrationsHeader, sqlStatements);
};
var $author$project$Generator$generateQueryFunctions = function (typeAlias) {
	var typeName = function (_v0) {
		var name = _v0.b;
		return name;
	}(typeAlias.name);
	var lowerTypeName = $elm$core$String$toLower(typeName);
	var pluralName = lowerTypeName + 's';
	var queries = A2(
		$elm$core$String$join,
		'\n',
		_List_fromArray(
			['create' + (typeName + 'Query : String'), 'create' + (typeName + ('Query = \"INSERT INTO ' + (pluralName + ' DEFAULT VALUES\"'))), '', 'get' + (typeName + 'Query : Int -> String'), 'get' + (typeName + ('Query id = \"SELECT * FROM ' + (pluralName + ' WHERE id = \" ++ String.fromInt id'))), '', 'getAll' + (typeName + 'sQuery : String'), 'getAll' + (typeName + ('sQuery = \"SELECT * FROM ' + (pluralName + '\"'))), '', 'delete' + (typeName + 'Query : Int -> String'), 'delete' + (typeName + ('Query id = \"DELETE FROM ' + (pluralName + ' WHERE id = \" ++ String.fromInt id')))]));
	var encoder = lowerTypeName + ('Encoder : ' + (typeName + (' -> Encode.Value\n' + (lowerTypeName + ('Encoder ' + (lowerTypeName + ' =\n    Encode.object []  -- TODO: Generate proper encoder'))))));
	var decoder = lowerTypeName + ('Decoder : Decode.Decoder ' + (typeName + ('\n' + (lowerTypeName + ('Decoder =\n    Decode.succeed ' + (typeName + '\n        |> Decode.hardcoded 0  -- TODO: Generate proper decoder'))))));
	return decoder + ('\n\n' + (encoder + ('\n\n' + queries)));
};
var $author$project$Generator$generateTypeDefinition = function (typeAlias) {
	var typeName = function (_v0) {
		var name = _v0.b;
		return name;
	}(typeAlias.name);
	var fetchedTypeName = 'Fetched' + typeName;
	return 'type alias ' + (fetchedTypeName + (' =\n    { ' + ($elm$core$String$toLower(typeName) + (' : ' + (typeName + '\n    , id : Int\n    , createdAt : Posix\n    , updatedAt : Posix\n    }')))));
};
var $author$project$Generator$generateQueries = function (typeAliases) {
	var typeDefinitions = A2(
		$elm$core$String$join,
		'\n\n',
		A2($elm$core$List$map, $author$project$Generator$generateTypeDefinition, typeAliases));
	var queryFunctions = A2(
		$elm$core$String$join,
		'\n\n',
		A2($elm$core$List$map, $author$project$Generator$generateQueryFunctions, typeAliases));
	var queriesHeader = 'module Queries exposing (..)\n\nimport Json.Decode as Decode\nimport Json.Encode as Encode\nimport Time exposing (Posix)\n\n';
	return queriesHeader + (typeDefinitions + ('\n\n' + queryFunctions));
};
...
var $author$project$Generator$sendMigrations = _Platform_outgoingPort('sendMigrations', $elm$json$Json$Encode$string);
var $author$project$Generator$sendQueries = _Platform_outgoingPort('sendQueries', $elm$json$Json$Encode$string);
var $author$project$Generator$init = function (ormFileContent) {
	var parseResult = A2(
		$elm$core$Result$map,
		...
		A2(
			...
			$stil4m$elm_syntax$Elm$Parser$parse(ormFileContent)));
	var commands = function () {
		if (parseResult.$ === 'Ok') {
			var file = parseResult.a;
			var typeAliases = $author$project$Generator$extractTypeAliases(file);
			var queries = $author$project$Generator$generateQueries(typeAliases);
			var migrations = $author$project$Generator$generateMigrations(typeAliases);
			return $elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						$author$project$Generator$sendQueries(queries),
						$author$project$Generator$sendMigrations(migrations)
					]));
		} else {
			var error = parseResult.a;
			return $elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						$author$project$Generator$sendQueries('-- Error: ' + error),
						$author$project$Generator$sendMigrations('-- Error: ' + error)
					]));
		}
	}();
	return _Utils_Tuple2(
...
var $author$project$Generator$update = F2(
	function (msg, model) {
		return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
	});
var $elm$core$Platform$worker = _Platform_worker;
var $author$project$Generator$main = $elm$core$Platform$worker(
	{
		init: $author$project$Generator$init,
		subscriptions: function (_v0) {
			return $elm$core$Platform$Sub$none;
		},
		update: $author$project$Generator$update
	});
_Platform_export({'Generator':{'init':$author$project$Generator$main($elm$json$Json$Decode$string)(0)}});}(this));