...
type alias UserDefinedTable = {
        name: String,
        columnDefs: List DynamicColumnDef, -- json type, gets encoded / decoded, just for user UI and dynamic data storage
        rows: Multilink Row
    }
type BacklinkToUserDefinedTable = Backlink UserDefinedTable
...
type alias RankResult = {
        reference: DynamicRow, -- the one thing that many other results are ranked against
...
--
-- [generator-start]
type DynamicColumnDef = DynamicColumnDef
type alias DynamicRow = {values: List DynamicValue}
type alias DynamicValue = Encode.Value
